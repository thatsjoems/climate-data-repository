...
| 11 | Password recovery workflow | **NOT YET IMPLEMENTED (Should-Have)** | Mentioned in the ICN as a required function; not built in this 8-week prototype |
...
| 13 | Institutional onboarding (focal person nomination) | **PARTIAL** | Admin can add institutions and users manually (`AdminPanel.tsx`); a full "nomination workflow" is not built |
| 14 | Audit logging | **IMPLEMENTED** | `AuditLog` model + `audit_service.py`, records LOGIN, SUBMISSION_CREATED, USER_CREATED, etc. |
| 15 | Climate risk assessment for the banking sector (exposure analysis) | **PARTIAL (Prototype indicator)** | `analytics_service.get_hazard_exposure()` - shows total loan value by region/hazard from actual submission data. **This is NOT a formal "climate risk score"** - it is a descriptive summary only |
